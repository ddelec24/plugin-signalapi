...
if(isset($received) && is_object($received)) {
	...
	//log::add('signal', 'debug', "dataMessage: " . $received->envelope->dataMessage->message);
		//log::add('signal', 'debug', "sentMessage: " . $received->envelope->syncMessage->sentMessage->message);
	

	$sourceNumber = 	$received->envelope->sourceNumber;
	$name = 			$received->envelope->sourceName;
	$timestamp = 		$received->envelope->timestamp;
		$msg = 				(!empty($received->envelope->dataMessage->message)) ? $received->envelope->dataMessage->message : $received->envelope->syncMessage->sentMessage->message; // suivant is on envoi à nous meme ou non
