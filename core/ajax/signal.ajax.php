...
try {
	...
	*/
		...
		if (init('action') == 'updateContacts') { 
			ajax::success(updateSignalContacts(init('eqLogic'), init('contacts')));
		}
...
function updateSignalContacts($eqLogic, $arrContacts) {
	$eq = signal::byId($eqLogic);
	$sender = trim($eq->getConfiguration("numero"));
  
	if(isset($arrContacts) && is_array($arrContacts)) {
      foreach($arrContacts as $k => $contact) {
        $arrContacts[$k]['display'] = ($contact['display'] == "true") ? true : false; // string to bool trick
      }

      $eq->setConfiguration('contactsList', $arrContacts);
      $eq->save(true);
	}
}