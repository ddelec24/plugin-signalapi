...
$portSignal = config::byKey('port', 'signal');
...
sendVarToJS('paramsSignal', $params);
...
<div class="row row-overflow">
	...
	<div class="col-xs-12 eqLogicThumbnailDisplay">
		...
		<legend><i class="fas fa-table"></i> {{Mes Numéros associés}}</legend>
		<?php
		if (count($eqLogics) == 0) {
			echo '<br><div class="text-center" style="font-size:1.2em;font-weight:bold;">{{Aucun numéro trouvé, cliquer sur "Ajouter un numéro" pour commencer}}</div>';
		} else {
			// Champ de recherche
				...
				<form class="form-horizontal">
					<fieldset>
						<div class="col-lg-6">
							...
							<div class="form-group">
								<label class="col-sm-4 control-label">{{Numéro de téléphone}}
									<sup><i class="fas fa-question-circle tooltips" title="{{Renseignez le numéro signal au format international comme indiqué sur votre téléphone avec le +33 pour la France, +32 pour la Belgique etc.}}"></i></sup>
								</label>
								<div class="col-sm-6">
									<input type="tel" class="eqLogicAttr form-control" data-l1key="configuration" data-l2key="numero" placeholder="{{+336XXXXXXXX}}" pattern="[+]{1}[0-9]{5,14}" />
								</div>
							</div>
                            <?php
                            if($displayLink) {
								...
								echo '<a href="http://localhost:'. (($portSignal == "") ? 8099 : $portSignal) .'/v1/qrcodelink?device_name=signal-api'. ($eqLogic->getId()).'" target="_blank"> Associer mon téléphone avec l\'Api</a> ';
